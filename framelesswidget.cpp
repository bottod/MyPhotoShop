...
FrameLessWidget::FrameLessWidget(QWidget *parent)
    : QWidget(parent)
{
    this->setWindowIcon(QIcon(":/Label_Image/V-Team"));//设置任务栏图标
...
{
    //整体背景色设置为透明 必须要设置 FramelessWindowHint 才有效
    this->setAttribute(Qt::WA_TranslucentBackground);

    //标题栏窗口 限制高度TitleHeight
    m_pTitleWidget->setFixedHeight(TitleHeight);

    //标题栏布局管理器
    m_pHTopLayout->addWidget(m_pTitleImgLabel);
    m_pHTopLayout->addWidget(m_pTitleTxtLabel);
    m_pHTopLayout->addStretch();
    m_pHTopLayout->addWidget(m_pTitleUserLabel);
...
{
    m_pTitleWidget = new QWidget();m_pTitleWidget->setObjectName("TitleWidget");                                                 //标题栏窗口
    m_pBottomWidget = new QWidget();m_pBottomWidget->setObjectName("BottomWidget");                                               //标题栏下面内容窗口
    ...
    //--------------------------------------
    m_pTitleTxtLabel = new QLabel("Title");m_pTitleTxtLabel->setObjectName("TitleTxt");
    m_pTitleTxtLabel->setFixedHeight(35);
    //-----------------------------------------------
    m_pTitleImgLabel = new QLabel;m_pTitleImgLabel->setObjectName("TitleImg");
    m_pTitleImgLabel->setFixedSize(70,41);
    m_pTitleImgLabel->setScaledContents(true);//可以进行缩放
    //-----------------------------------------------
    m_pTitleUserLabel = new QLabel("<a href = www.baidu.com >帮助</a>",this); //添加网页标记
    m_pTitleUserLabel->setOpenExternalLinks(true); //快速实现打开链接 去掉了绑定信号
    m_pTitleUserLabel->setObjectName("TitleUser");
    m_pTitleUserLabel->setFixedHeight(40);
...
//设置标题栏相关信息，待补充
void FrameLessWidget::SetTitleInfo(int title_height)
{
...
//返回鼠标相对窗口的位置
QPoint FrameLessWidget::GetMousePosInWindow()
{
    return m_MovePosition;
}
//返回标题栏的高度
int FrameLessWidget::GetTitleHeigit()
{
    return TitleHeight;
}
//返回内容窗口
QWidget* FrameLessWidget::GetContentWidget()
{
    return m_pBottomWidget;
}

//返回标题栏窗口
QWidget* FrameLessWidget::GetTitleWidget()
{
    return m_pTitleWidget;
}

//返回窗口图标标签
QLabel* FrameLessWidget::GetTitleImgLabel()
{
    return m_pTitleImgLabel;
}

//返回窗口名字标签
QLabel* FrameLessWidget::GetTitleTxtLabel()
{
    return m_pTitleTxtLabel;
}

//返回窗口超链接标签
QLabel* FrameLessWidget::GetTitleUserLabel()
{
    return m_pTitleUserLabel;
}
