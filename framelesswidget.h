...
public:
    ...
    void SetTitleInfo(int title_height);            //设置标题栏信息
    QPoint GetMousePosInWindow();                   //返回鼠标相对窗口的位置
    int GetTitleHeigit();                           //返回标题栏的高度
    QWidget* GetContentWidget();                    //返回内容窗口进行操作
    QWidget* GetTitleWidget();                      //返回标题窗口进行操作
    QLabel * GetTitleTxtLabel();                    //返回窗口名字标签
    QLabel * GetTitleImgLabel();                    //返回窗口图标标签
    QLabel * GetTitleUserLabel();                   //返回窗口超链接标签
...
private:
    ...
    QLabel * m_pTitleTxtLabel;                          //标题栏标签(文字)
    QLabel * m_pTitleImgLabel;                          //标题栏标签(图片)
    QLabel * m_pTitleUserLabel;                         //标题栏标签(信息超链接)
    //按钮类型
